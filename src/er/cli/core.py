...
def extract() -> None:
    ...
    packer.unpack("workspace/FAL_PS.scr", "workspace/raw/FAL_PS.json")
...
def replace(check: bool = True) -> None:
    ...
    packer.pack(
        "workspace/generated/translated/FAL_PS.json",
        "workspace/generated/dist/FAL_PS_chs.scr",
    )
    ...
    text_hook.TextHookBuilder(os.environ["TEXT_HOOK_PROJECT_PATH"]).build(
        FEATURES, panic="immediate-abort", output_name="FAL_PS_chs.dll"
    )
