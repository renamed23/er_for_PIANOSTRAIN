...
def unpack(input_path: PathLike, out_path: PathLike) -> None:
    ...
    b = invert_bytes(source.read_bytes())
    ...
    ctrl_15 = cast(U32, de(data_json["ctrl"][15]))
    ctrl_16 = cast(U32, de(data_json["ctrl"][16]))
    if ctrl_15 >= 0:
        data_json["palette_data"] = []
        for _ in range(ctrl_16):
            data_json["palette_data"].append(se(reader.read_u32()))

    for _ in range(13):
        data_json["ctrl"].append(se(reader.read_u32()))

    data_json["active_states_init"] = []
    ctrl_0 = cast(U32, de(data_json["ctrl"][0]))
    for _ in range(ctrl_0):
    ...
    for _ in range(cast(U32, de(data_json["ctrl"][19]))):
        data_json["jump_table"].append(se(reader.read_u32()))

    data_json["var_pool_init"] = []
    for _ in range(cast(U32, de(data_json["ctrl"][1]))):
        data_json["var_pool_init"].append(se(reader.read_u32()))

    data_json["var_pool_ext_init"] = []
    for _ in range(cast(U32, de(data_json["ctrl"][2]))):
        data_json["var_pool_ext_init"].append(se(reader.read_u32()))

    data_json["resource_info"] = []
    for _ in range(cast(U32, de(data_json["ctrl"][3]))):
    ...
    for _ in range(cast(U32, de(data_json["ctrl"][4]))):
    ...
    for _ in range(cast(U32, de(data_json["ctrl"][5]))):
    ...
    ctrl_6 = cast(U32, de(data_json["ctrl"][6]))
    if ctrl_6 > 0:
        res_len = reader.read_u16()
        data_json["res_path_a"] = se(reader.read_bytes(res_len))

    ctrl_7 = cast(U32, de(data_json["ctrl"][7]))
    if ctrl_7 > 0:
        res_len = reader.read_u16()
    ...
    if has_meta != 0:
        string_len = reader.read_u16()
        string_data_reader = BinaryReader(reader.read_bytes(string_len))
        data_json["string_data"] = []
        while not string_data_reader.is_eof():
            next_string_len = string_data_reader.read_u8()
            string = string_data_reader.read_bytes(next_string_len)
            assert string[-1] == 0x00
            data_json["string_data"].append(string[:-1].decode("CP932"))

        version = cast(U32, reader.read_u32())
        data_json["version"] = se(version)
        data_json["version_data"] = se(reader.read_bytes(version * ctrl_0))

    bytecode_len = reader.read_u32()
    bytecode = reader.read_bytes(bytecode_len)
    assert reader.is_eof()
    ctrl_28 = cast(U32, de(data_json["ctrl"][28]))
    data_json["bytecode_blocks"] = se(Bytes(bytecode[:ctrl_28]))

    bytecode = bytecode[ctrl_28:]
    data_json["bytecode"] = []
    ...
    while i < total_len:
        ...
        if byte == 0x23:
            ...
            data_json["bytecode"].append({
                "offset": last_push_offset + ctrl_28,
...
def pack(input_path: PathLike, out_path: PathLike) -> None:
    ...
    Args:
        ...
        out_path: 输出包路径。
    """
    input = to_path(input_path)
    output_path = to_path(out_path)
    data_json = read_json(input)

    # --- 1. 预处理：重新组装 Bytecode 并计算偏移映射 ---
    # bytecode_blocks 的长度是固定的，由 ctrl[28] 定义
    bytecode_blocks_raw = cast(Bytes, de(data_json["bytecode_blocks"]))
    ctrl_28 = len(bytecode_blocks_raw)

    new_bytecode_body = bytearray()
    old_to_new = {}

    # 初始偏移量必须加上 bytecode_blocks 的长度，因为指针是相对于整个 bytecode 区域的
    current_new_offset = ctrl_28
    ...
    def remap_ptr(old_ptr: int) -> int:
        """严格的指针重映射逻辑"""
        if old_ptr < ctrl_28:
            # 指向固定的 blocks 区域，不需要重映射
            return old_ptr
        if old_ptr in old_to_new:
            # 指向动态 bytecode 区域，返回映射后的新偏移
            return old_to_new[old_ptr]
        # 既不在静态区也不在映射表，说明解包数据或偏移逻辑有误
        raise ValueError(
            f"检测到非法指针: {old_ptr}。该偏移既不属于静态块(size={ctrl_28})，也不在字节码映射表中。"
        )

    # --- 2. 映射更新 ---
    # 映射 jump_table
    updated_jump_table = [
        remap_ptr(int(de(entry))) for entry in data_json["jump_table"]
    ]

    # 映射 active_states_init 的第一个 U32
    updated_active_states = []
    for pair in data_json["active_states_init"]:
        old_ptr = int(de(pair[0]))
        updated_active_states.append([remap_ptr(old_ptr), int(de(pair[1]))])
    ...
    for i in range(17):
        writer.write_u32(ctrl_values[i])

    # Palette Data
    if "palette_data" in data_json:
        for p_val in data_json["palette_data"]:
            writer.write_u32(int(de(p_val)))
    ...
    # Active States Init
    for ptr, val in updated_active_states:
        writer.write_u32(ptr)
        writer.write_u32(val)

    # Jump Table
    for addr in updated_jump_table:
    ...
    # Resource Paths
    if "res_path_a" in data_json:
        p_a = cast(Bytes, de(data_json["res_path_a"]))
        writer.write_u16(len(p_a))
        writer.write_bytes(p_a)

    if "res_path_b" in data_json:
        p_b = cast(Bytes, de(data_json["res_path_b"]))
        writer.write_u16(len(p_b))
        writer.write_bytes(p_b)
    ...
    if has_meta != 0:
        # 重组 string_data 块
        meta_writer = BinaryWriter()
        for s in data_json["string_data"]:
            s_bytes = s.encode("CP932") + b"\x00"
            meta_writer.write_u8(len(s_bytes))
            meta_writer.write_bytes(s_bytes)

        meta_blob = meta_writer.to_bytes()
        writer.write_u16(len(meta_blob))
        writer.write_bytes(meta_blob)

        # Version (U32) & Version Data
        writer.write_u32(int(de(data_json["version"])))
        writer.write_bytes(cast(Bytes, de(data_json["version_data"])))

    # Bytecode
    # 最终长度是 blocks + 重新组装的 body
    full_bytecode = bytecode_blocks_raw + new_bytecode_body
    writer.write_u32(len(full_bytecode))
    writer.write_bytes(full_bytecode)
    ...
    console.print(
        f"[OK] pack 完成: {input} -> {output_path}",
        style="info",
    )
